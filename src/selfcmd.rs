//! `ewg self` - manage the installed binary itself.
//!
//! `self update` shells out to `cargo install easywireguard --force`, the crate name even when the command you type is shorter.
...
fn update(yes: bool) -> Result<()> {
    ...
    println!(
        "{} {}\n",
        "Updating easywireguard via".dimmed(),
        "cargo install easywireguard --force".bold()
    );
        ...
        .status()
    {
        Ok(status) if status.success() => {
            println!("\n{}", "✓ easywireguard is up to date.".green());
            Ok(())
        }
...
fn check() -> Result<()> {
    ...
    if newer(&latest, current) {
        println!(
            "{} {} {}",
            format!("easywireguard {latest}").bold(),
            "is available, you have".dimmed(),
            current.bold()
        );
        println!("{} {}", "run".dimmed(), "ewg self update".bold());
    } else {
        println!(
            "{} {}",
            format!("easywireguard {current}").bold(),
...
/// `cargo search` prints `easywireguard = "X.Y.Z"    # description` for an exact name
...
fn confirm() -> bool {
    print!(
        "{} {} ",
        "Update easywireguard to the latest release via cargo?".bold(),
        "[y/N]".dimmed()
    );
